...
TEST(SelectorMatches, SimpleMath)
{
  ...
  const char* html_source = "<h1>"
                            "</h1>";

  const char* css_source = "h1 {"
                           "color: black;"
                           "}"
                           ""
                           "h2 {"
  ...
  yahtml::Element* h1_elem =
      dynamic_cast<yahtml::Element*>(htmldriver.dom.get());
...
TEST(MatchingRules, UniversalSelector)
{
  yahtml::HTMLDriver htmldriver;
  yacss::CSSDriver cssdriver;
  ...
  ASSERT_EQ(body->children.size(), 2);
  yahtml::Element* h1 =
      dynamic_cast<yahtml::Element*>(body->children.at(0).get());
  yahtml::Element* h2 =
      dynamic_cast<yahtml::Element*>(body->children.at(1).get());

  DeclarationContainer body_decls =
      compute_specified_values(cssdriver.stylesheet, *body);
  DeclarationContainer h1_decls =
      compute_specified_values(cssdriver.stylesheet, *h1);
  DeclarationContainer h2_decls =
      compute_specified_values(cssdriver.stylesheet, *h2);

  ASSERT_EQ(body_decls.size(), 1);
  EXPECT_EQ(body_decls.at("display").get<KeywordValue>().val, "block");

  ASSERT_EQ(h1_decls.size(), 1);
  EXPECT_EQ(h1_decls.at("display").get<KeywordValue>().val, "block");

  ASSERT_EQ(h2_decls.size(), 1);
  EXPECT_EQ(h2_decls.at("display").get<KeywordValue>().val, "inline");
}
